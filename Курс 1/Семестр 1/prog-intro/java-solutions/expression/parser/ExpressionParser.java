...
public class ExpressionParser extends BaseParser implements TripleParser {
    ...
    private ExpressionElement parseBitOr() {
        ExpressionElement temp = parseBitXor();
        while (true) {
            if (take('|')) {
    ...
    private ExpressionElement parseBitXor() {
        ExpressionElement temp = parseBitAnd();
        while (true) {
            if (take('^')) {
    ...
    private ExpressionElement parseBitAnd() {
        ExpressionElement temp = parseThirdPriorityOperations();
        while (true) {
            if (take('&')) {
    ...
    private ExpressionElement parseThirdPriorityOperations() {
        ExpressionElement temp = parseSecondPriorityOperations();
        while (true) {
            if (take('+')) {
    ...
    private ExpressionElement parseSecondPriorityOperations() {
        skipWhitespaces();
        ExpressionElement temp = parseFirstPriorityOperations();
        skipWhitespaces();
        while (true) {
            if (take('*')) {
                skipWhitespaces();
    ...
    private ExpressionElement parseFirstPriorityOperations() {
        ExpressionElement temp;
        if (take('-')) {
            if (take('(')) {
    ...
    private ExpressionElement parseVarConst(String prefix) {
